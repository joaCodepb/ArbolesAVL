...
public class ArbolExpresionGrafico<T> extends JPanel {
    private final ArbolB<T> miArbol;
    ...
    /**
     * Constructor que permite inicializar los atributos de la clase ArbolExpresionGrafico
     * y llama al método repaint(), que es el encargado de pintar el Arbol.
     * @param miArbol - Arbol creado
     */
     ...
     * forma parte del subárbol, para conocer en que posición van a ir dibujados
     */
    ...
    private void calcularPosicion(NodoB<T> n, int left, int right, int top){
      ...
      int width = fm.stringWidth(n.getValor().toString());
    ...
    private void dibujarArbol(Graphics2D g, NodoB<T> n, int puntox, int puntoy, int yoffs){
        ...
        g.drawString(n.getValor().toString(), r.x + 3, r.y + yoffs);
